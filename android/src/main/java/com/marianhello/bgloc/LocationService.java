...
public class LocationService extends Service {
    ...
    private static String PREX = "----->>>>> location service";

    private static String ACTION_FROMBGLOCATION_SERVICE = "bg location service";
...
//    private BgLocationBinder binder;
//    private MyConn conn;
...
//        }
    ...
    public void onDestroy() {
        log.info(TAG, PREX + "Destroying LocationService");
        provider.onDestroy();
     ...
     * bind service
     */
//    class BgLocationBinder extends ProcessService.Stub {
//        @Override
//        public String getServiceName() throws RemoteException {
//            String packageName = getPackageName();
//            return packageName+":bgLocLocation";
//        }
//    }
...
//            log.error(TAG,"与GuardService连接成功");
...
//            // 启动FirstService
//            Intent guardIntent = new Intent(getApplicationContext(), BgLoccGuardService.class);
//            if (config == null){
//                try {
...
//            LocationService.this.startService(guardIntent);
//            //绑定FirstService
//            LocationService.this.bindService(new Intent(LocationService.this, BgLoccGuardService.class),conn, Context.BIND_IMPORTANT);
//        }
//    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info(TAG, PREX + "Received start startId: {} intent: {}", startId, intent);
...
//        bindService(new Intent(this, BgLoccGuardService.class), conn, Context.BIND_IMPORTANT);
    ...
    public void handleLocation(BackgroundLocation location) {
        ...
        bundle.putString("action", ACTION_FROMBGLOCATION_SERVICE);
    ...
    private class PostLocationTask extends AsyncTask<BackgroundLocation, Integer, Boolean> {
        ...
        protected Boolean doInBackground(BackgroundLocation... locations) {
            Log.e(TAG,"Executing PostLocationTask#doInBackground");
    ...
    private void keepLocationService(){
        ...
        if (!isRunning) {
            log.error(TAG, "  ------->>>>>>>  guard service stopped restarting");
            try {
                Intent guardIntent = new Intent(getApplicationContext(), BgLoccGuardService.class);
                if (config == null){
                    ConfigurationDAO dao = DAOFactory.createConfigurationDAO(this);
                ...
                Context context = getApplicationContext();
                if (context != null) {
                    Log.e(TAG, " --->>>>> double guard guard service start");
                    context.startService(guardIntent);
                } else {
                    Log.e(TAG, " --->>>>> double guard context is null");
                }
            } catch (Exception e) {
                Log.e(TAG, " ----->>>>> double guard start guardservice from location service fail");
                e.printStackTrace();
            }
        }
    }
