...
import com.marianhello.bgloc.BgLoccGuardService;
...
public class DistanceFilterLocationProvider extends AbstractLocationProvider implements LocationListener {
    ...
    private static final String AWAKE_PERIODIC_ACTION              = P_NAME + ".AWAKE_PERIODIC_ACTION";
    ...
    private PendingIntent awakePeriodicPI;
    ...
    private static String processName = ":bgLocLocGuard";
    private static String selfProcessName = ":bgLocLocation";
    ...
    public void onCreate() {
        ...
        context = locationService.getApplicationContext();
        ...
        // awake device PI
        awakePeriodicPI = PendingIntent.getBroadcast(context, 0, new Intent(AWAKE_PERIODIC_ACTION), 0);
        registerReceiver(awakeDeviceReceiver, new IntentFilter(AWAKE_PERIODIC_ACTION));


        long periodic = 15 * 1000;

        if (config.getInterval() != null && config.getInterval() >= 0){
            periodic = config.getInterval();
        }
        alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, System.currentTimeMillis(), periodic, awakePeriodicPI);
        ...
        //设置播放管理器
//        mediaPlayer = MediaPlayer.create(locationService, ToneGenerator.TONE_PROP_BEEP);
//        mediaPlayer.setWakeMode(context, PowerManager.PARTIAL_WAKE_LOCK);
    ...
    */
    ...
    /**
     * @Author JimmyDaddy
     * @decription 定时程序广播接收器，播放音乐保持唤醒，同时检测服务是否运行，然后判断是否启动
     */
    private BroadcastReceiver awakeDeviceReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
//            mediaPlayer.prepareAsync();
//            mediaPlayer.start();
//            mediaPlayer.pause();
            Log.e("sxc location ", TAG + "--->>>   onReceive  LOCATION_CLOCK");
            String packageName = context.getPackageName();
            boolean isRun = ServiceRunning.isRunning(context,packageName+selfProcessName);

            if (!isRun) {
                Intent locationIntent = new Intent(context, LocationService.class);

                if (config != null) {
                    Log.e("sxc location ", TAG + "--->>>   onReceive  service stopped LocationProvider"+config.getLocationProvider());
                    locationIntent.putExtra("config", config);
                }

                Location location = getCurrentPosition();
                if (location != null){
                    Log.e("sxc location ", TAG + "--->>>   onReceive  service stopped getCurrentPosition "+location.getLatitude());
                    handleLocation(location);
                }
                context.startService(locationIntent);
            } else {
                Log.e("sxc location ", TAG + "--->>>   onReceive  service running");
//                Location location = getCurrentPosition();
//                if (location != null){
//                    Log.d("sxc location ", "--->>>   onReceive  service running force getCurrentPosition "+location.getLatitude());
//                    handleLocation(location);
//                }
            }
            keepLocationService();
    ...
    private void keepLocationService(){
        if (context != null) {
            String packageName = context.getPackageName();
            Boolean isRunning = ServiceRunning.isRunning(context, packageName+processName);
            if (!isRunning) {
                log.error("sxc location", TAG+"  ------->>>>>>>  guard service stopped restarting");
                try {
                    Intent guardIntent = new Intent(context, BgLoccGuardService.class);
                    if (config == null){
                        ConfigurationDAO dao = DAOFactory.createConfigurationDAO(context);
                        try {
                            config = dao.retrieveConfiguration();
                        } catch (JSONException e) {
                            log.error("sxc location", TAG + "  ------->>>>>>> Config exception: {}", e.getMessage());
                            config = new Config(); //using default config
                        }
                    }
                    guardIntent.putExtra("config", config);
                    guardIntent.addFlags(Intent.FLAG_FROM_BACKGROUND);
                    if (context != null) {
                        Log.e("sxc location", TAG+" --->>>>> double guard guard service start");
                        context.startService(guardIntent);
                    } else {
                        Log.e("sxc location", TAG + " --->>>>> double guard context is null");
                    }
                } catch (Exception e) {
                    Log.e("sxc location",TAG+" ----->>>>> double guard start guardservice from location service fail");
                    e.printStackTrace();
                }
            }
        }
...
//        mediaPlayer.release();
    ...
    public Location getCurrentPosition() {
        ...
        if(location == null){
            Log.e(TAG, "onCreate.location = null");
