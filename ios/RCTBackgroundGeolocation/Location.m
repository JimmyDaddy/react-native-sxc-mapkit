...
- (BOOL) postAsJSON:(NSString*)url withHttpHeaders:(NSMutableDictionary*)httpHeaders withParams:(NSMutableDictionary*)params error:(NSError * __autoreleasing *)outError;
{
...
//    NSData* paramsData = nil;
    NSMutableDictionary* allData = nil;
    if (params != nil && [params count]) {
//        paramsData = [NSJSONSerialization dataWithJSONObject:params options:0 error: outError];
        allData = [[NSMutableDictionary alloc] initWithObjectsAndKeys:locations, @"locations", params, @"params", nil];
    } else {
        allData = [[NSMutableDictionary alloc] initWithObjectsAndKeys:locations, @"locations", nil];
    }
    NSData* allDataFinal = nil;
    
    if ([NSJSONSerialization isValidJSONObject:allData]) {
         allDataFinal =  [NSJSONSerialization dataWithJSONObject:allData options:0 error: outError];
    }
    NSString *jsonStr = [[NSString alloc] initWithData:allDataFinal encoding:NSUTF8StringEncoding];
