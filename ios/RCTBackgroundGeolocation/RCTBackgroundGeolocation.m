...
RCT_EXPORT_METHOD(getCurrentPosition:(RCTPromiseResolveBlock)resolve
    ...
    Location* location =  [locationManager getCurrentLocation: &error];
    
    if (location == nil || error != nil) {
        reject(@"1001", @"位置获取失败", nil);
    }
 ...
 *addAppToWhiteList
 **/
RCT_EXPORT_METHOD(addAppToWhiteList:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    RCTLogInfo(@"RCTBackgroundGeolocation #addAppToWhiteList");
}

RCT_EXPORT_METHOD(getManufacture:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    resolve(@"apple");
}
