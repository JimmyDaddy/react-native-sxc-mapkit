    ...
    if ([_config hasSyncUrl] || [_config hasUrl]) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            if (hasConnectivity && [_config hasUrl]) {
                NSError *error = nil;
                if ([location postAsJSON:_config.url withHttpHeaders:_config.httpHeaders withParams:_config.params error:&error]) {
                    SQLiteLocationDAO* locationDAO = [SQLiteLocationDAO sharedInstance];
                    if (location.id != nil) {
 ...
 *
 */
- (Location* ) getCurrentLocation:(NSError * __autoreleasing *)outError
{
    NSUInteger authStatus;
    ...
    if (location != nil) {
        return [Location fromCLLocation:location];
    }
    return nil;
}
